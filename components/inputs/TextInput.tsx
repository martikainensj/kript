...
export const TextInput: React.FC<Props> = ({
  ...
  const rightIsVisible = !disabled && editable && value?.toString() && inputValue?.toString();
  ...
  const onChangeTextHandler = (string: string) => {
    if (keyboardType === "numeric") {
      ...
      if (wholePart?.length > 12 || decimalPart?.length > 12 || string.split(".").length > 2)
        return;
  ...
  useEffect(() => {
    ...
    if (keyboardType === "numeric") {
      const numberValue = !!inputValue ? parseFloat(inputValue.toString()) : "";
  ...
  return (
    ...
    >
      ...
      >
        <Text fontSize="sm" style={[styles.labelContainer, { top: -labelHeight * 0.5 }]}>
          {label}
        </Text>
      </Animated.View>
      ...
      <Pressable onPress={() => ref.current?.focus()}>
        ...
        >
          <RNTextInput
            ...
            style={[styles.inputContainer, { color: theme.colors.onBackground }, rest.style]}
          />

          {!!suffixComponent && (
            <View style={[!rightIsVisible && { marginRight: 12 }]}>{suffixComponent}</View>
          )}
